...
class author_post extends WP_Widget {
     function author_post() {
         ...
         parent::__construct('author_post', '作者模块', $widget_ops);
     }
...
<div class="widget widget_author">
		...
		global $post;
		  $args = array( 
			...
			?>
			<li>
				<a href="<?php $author_link = get_the_author_posts_link(); $author_link = preg_replace('/(.*)href="(.*)"(.*)/', '$2', $author_link);echo $author_link;?>" title="更多作者文章">
...
add_action( 'widgets_init', create_function( '', 'register_widget( "author_post" );' ) );

//分类目录模块
class cat_list_post extends WP_Widget {
     function cat_list_post() {
         $widget_ops = array('description' => '显示侧边带二级标签的分类栏目。');
         parent::__construct('cat_list_post', '分类栏目', $widget_ops);
     }
     function widget($args, $instance) {
		extract($args);
...
add_action( 'widgets_init', create_function( '', 'register_widget( "cat_list_post" );' ) );


//编辑推荐
class bj_like_post extends WP_Widget {
     function bj_like_post() {
         $widget_ops = array('description' => '显示侧边调用文章自定义字段。');
         parent::__construct('bj_like_post', '编辑推荐', $widget_ops);
     }
     function widget($args, $instance) {
		extract($args);
...
add_action( 'widgets_init', create_function( '', 'register_widget( "bj_like_post" );' ) );

//热门专题
class zhuanti_huo_post extends WP_Widget {
     function zhuanti_huo_post() {
         $widget_ops = array('description' => '显示侧边调用自定义文章类型。');
         parent::__construct('zhuanti_huo_post', '热门专题', $widget_ops);
     }
     function widget($args, $instance) {
		extract($args);
...
add_action( 'widgets_init', create_function( '', 'register_widget( "zhuanti_huo_post" );' ) );