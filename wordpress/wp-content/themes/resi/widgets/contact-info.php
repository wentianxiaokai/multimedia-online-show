...
class resi_Contact_Info extends WP_Widget {  



// constructor

    function resi_contact_info() {

		$widget_ops = array('classname' => 'resi_contact_info_widget', 'description' => esc_html__( 'Display your contact info', 'resi') ); 

        parent::__construct(false, $name = esc_html__('MT - Contact Info Widget', 'resi'), $widget_ops); 

		$this->alt_option_name = 'resi_contact_info'; 
	...
	<p><label for="<?php echo sanitize_text_field( $this->get_field_id( 'address' )); ?>"><?php esc_html_e( 'Enter your address', 'resi' ); ?></label>
	...
	<p><label for="<?php echo sanitize_text_field( $this->get_field_id( 'phone' )); ?>"><?php esc_html_e( 'Enter your phone number', 'resi' ); ?></label>
	...
	<p><label for="<?php echo sanitize_text_field( $this->get_field_id( 'email' )); ?>"><?php esc_html_e( 'Enter your email address', 'resi' ); ?></label>
	...
	function update($new_instance, $old_instance) {
		...
		if ( isset($alloptions['resi_contact_info']) )

			delete_option('resi_contact_info');		  
	...
	function flush_widget_cache() {
		...
		wp_cache_delete('resi_contact_info', 'widget');
	...
	function widget($args, $instance) {
		...
		if ( ! $this->is_preview() ) {
			...
			$cache = wp_cache_get( 'resi_contact_info', 'widget' );
		...
		$title = ( ! empty( $instance['title'] ) ) ? esc_attr( $instance['title'] ) : esc_html__( 'Contact info', 'resi' );
		...
		if ( $title ) echo $before_title . wp_kses_post( $title ) . $after_title; 
		...
		if( ($address) ) {
			...
			echo '<span>' . esc_html__('Address: ', 'resi') . '</span>' . wp_kses_post( $address );
		...
		if( ($phone) ) {
			...
			echo '<span>' . esc_html__('Phone: ', 'resi') . '</span>' . wp_kses_post( $phone );
		...
		if( ($email) ) {
			...
			echo '<span>' . esc_html__('Email: ', 'resi') . '</span>' . '<a href="mailto:' . sanitize_email( $email ) . '">' . sanitize_email( $email ) . '</a>';
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'resi_contact_info', $cache, 'widget' );  
